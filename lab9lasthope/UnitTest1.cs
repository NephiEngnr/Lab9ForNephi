...
List<string> MadLibWords = new();
TheText = File.ReadAllText("Story.txt");
// Console.WriteLine(TheText);

//Now checking each word for special character.
;
foreach (string thisWord in TheText.Split(' '))
{
    if (thisWord.StartsWith('^'))
    {
        // Console.WriteLine(thisWord);
        Answers.Add(PromptUser(thisWord));
    }
}

foreach (string x in Answers) Console.WriteLine(x);

string PromptUser(string Lstring)
{
    //Instructions
    Console.Write("Please enter a(n)" + Lstring + ": ");
...
// The next day, a(n) {noun2} really did be there. ";// {verb4}";//his sheep and he {^past-tense-verb} and {^past-tense-verb} to call for help,
